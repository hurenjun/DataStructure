...
public class LinkCutTree {
	...
	public void link(int v, int w, float cost) {
		access(v);
		access(w);
		left[v] = w;
		parent[w] = v;
	        ...
	        if (cmd == 0) {
				...
				while (true) {
					...
					if (lct.findRoot(v) != lct.findRoot(w)) {
						...
						lct.link(root, w, (float) (rnd.nextFloat() + 0.5));
						break;
					}
					trial++;
		        	if (trial == 1000) break;
